...
func unlock(ctx *cli.Context) error {
	...
	// Should the daemon be initialized stateless?
	statelessInit := ctx.IsSet("stateless_init") &&
		ctx.Bool("stateless_init")
...
func changePassword(ctx *cli.Context) error {
	...
	// If stateless initialization was requested, there can be an optional
	// --save_to parameter that specifies where the returned macaroon
	// should be saved to.
	var macSavePath string
	if ctx.IsSet("save_to") {
	...
	// Should a new macaroon root key be generated by the daemon?
	newMacRootKey := ctx.IsSet("new_mac_root_key") &&
		ctx.Bool("new_mac_root_key")

	req := &lnrpc.ChangePasswordRequest{
		CurrentPassword:    currentPw,
		NewPassword:        newPw,
		StatelessInit:      statelessInit,
		NewMacaroonRootKey: newMacRootKey,
	}
	...
	// If stateless initialization was requested, we receive a macaroon
	// in the response. The user MUST store this macaroon somewhere so we
	// either save it to a provided file path or just print it to standard
	// output.
	if statelessInit {
