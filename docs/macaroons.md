...
As mentioned above, by default `lnd` creates three macaroon files in it's
...
* Create a new wallet stateless (first run):
  * `lncli create --stateless_init --save-to=/safe/location/admin.macaroon`
* Unlock a wallet that has previously been initialized stateless:
  * `lncli --macaroonpath=/safe/location/admin.macaroon unlock --stateless_init`
