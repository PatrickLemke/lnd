...
func testStatelessInit(net *lntest.NetworkHarness, t *harnessTest) {
	var (
		ctxb        = context.Background()
		timeout     = time.Duration(time.Second * 15)
		timeoutLong = time.Duration(time.Second * 30)
		ctxt, _     = context.WithTimeout(ctxb, timeout)
		ctxtLong, _ = context.WithTimeout(ctxb, timeoutLong)
	...
	res, err := adminMacClient.NewWitnessAddress(ctxt, newAddrReq)
	if err != nil {
	...
	// Changing the password takes a while, so we use a longer timeout
	// (30 seconds instead of only 15) to wait for the connection to be
	// ready.
	res, err = adminMacClient.NewWitnessAddress(ctxtLong, newAddrReq)
	if err != nil {
		t.Fatalf("unable to get new address with macaroon: %v", err)
	}
	if !strings.HasPrefix(res.Address, "r") {
